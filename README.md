...
N - это степень дерева. Если степень B-дерева равна N, то каждый из узлов может иметь максимум N потомков и N - 1 значений(напомню, что узел это массив). Все узлы, кроме корня должны иметь как минимум (N/2) – 1 элементов(деление округляем вверх). 
